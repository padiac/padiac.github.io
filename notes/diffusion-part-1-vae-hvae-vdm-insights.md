...
当我们在像素空间 $[0,1]^N$ 中讨论“图像的分布”时，原本的空间只有一个最自然的测度：**平直测度**（即所有像素组合等概率）。  
这一测度对应的是“最大无知”的假设，即：
...
### 训练数据会在原本的均匀空间上重新定义一个测度

引入真实图像数据后，像素空间的原始均匀测度不再成立。  
...
- 空间中“数据密集”的区域被赋予更高的测度  
- “数据稀疏”的区域测度极低  
- 整个像素空间被训练数据“重新塑形”  

简单说：  
**数据本身决定了空间里哪些点“重要”，哪些点“几乎不会出现”。**

---

### 模型训练的核心：通过样本反推出这个新的测度，这个时候我们是通过优化z来找到这个真正的测度，从而跳出均匀分布这个假设。举个例子，比如DDPM其实是将原本的图像分布，映射到一个高斯分布。这就类似于艺术家看到未经雕琢的大理石的就能想象出作品的样子，每一个随机的高斯分布也对应一个真实的图像。

生成模型的训练本质上不是“拟合概率”，而是：

> **根据样本反推像素空间真正的测度分布。**

最大似然、扩散模型、VAE 等方法，本质上都是在做同一件事：

- 给定一批样本 $\{x^{(i)}\}$  
- 寻找一个新的 $p_\theta(x)$  
- 使得这些样本在这个新测度下“看起来最典型、最自然”

也就是说：

> **训练 = 让模型找到一个测度，使得训练数据成为高测度区域。**
...
这里要先澄清一件事：像素空间上的 $p(x)$ 本身其实是**算得出来的**。给我一亿张图片，我完全可以逐 pixel 统计亮度直方图，再把各个 pixel 的边缘分布乘起来，就得到一个合法的、可采样的分布。它既不是尖峰，也不需要任何近似。

问题不在于能不能算，而在于**这样算出来的分布没有意义**：它假设 pixel 之间彼此独立，所以采样出来只是彩色噪声。而我们想要的从来不是 general 的图像分布，而是“有意义的图像”的分布——比如一只猫对应的那一小片区域。“意义”恰恰藏在 pixel 之间的耦合里，而逐 pixel 的边缘分布看不见任何耦合。

于是我们引入 latent 变量 $z$，把分布写成

$$ p(x) = \int p(x \mid z)p(z) dz = \int p_\theta(x \mid z)p(z) dz. $$

这一步的收获**不是**“得到了一个更好算的 $p(x)$”，而是：我们真正想要的东西已经不再是 $p(x)$ 了，而是括号里那两个条件分布——

- $p(x \mid z)$：给定 latent 之后的图像分布，也就是 **decoder**
- $p(z \mid x)$：给定图像之后的 latent 分布，也就是 **encoder**

这两个才是我们要学的对象。$p(x)$ 只是把它们串起来的中间量，训练时我们既不需要它的数值，也不会去求它。而它在这个 latent 形式下反而变得不可算了：可算的那个 $p(x)$ 没有意义，有意义的那个 $p(x)$ 不可算。我们的出路是绕开它，直接去学 encoder 和 decoder。
...
4. 文字可以轻易的转化成embedding vector. 例如一个k dimension的向量空间，这时候应当认为这个空间是完备的，也就是文字可以取所有值。$p(z)$这时候就是先验分布。在贝叶斯中，我们有很多取先验的方式，其中正太分布是非常常见的一种, 下面的讨论中可以看到$p(z)$的确用的就是正态分布。比如如果认为z满足正太分布$N(\mu, \sigma^2)$, 那么在不知道$\mu$的情况下，我们会为$\mu$也选取一个正太分布$N(\tau, \theta^2)$。简单说就是学文字表示，学低维表示，学相同维度的噪声，其实都是同一件事情。
5. z不一定是文字表示，它可以是任何低维度的图片表示，这可以看成是原始图片的信息压缩。但也可以是相同size的噪声， VDM就是这个做法, DDPM也是。反过来，我们不一定需要文字，因为文字是图像的子集，也就是图像可以包含所有文字的意义。举个例子，如果我把所有的文字拍个照片，可以直接把文字转换成图像。
6. $p_\theta(x \mid z)$ 是给定文字之后的图像分布，也就是decoder. $p(z \mid x)$ 是给定图像之后的文字分布，也就是encoder. 这两个都是我们需要训练得到的结果，因为实际上的stable diffusion, 为了减少计算量，我们使用的是先将encoder压缩图像到latent space，做扩散，之后再upsample到原图，这个之后会细说。 
...
这个$ q_\phi(z \mid x)$，其实就是我们就是我们之后在DDPM里面要学的噪声。它可以通过Unet或者DiT计算出来。另外，$p(z)$作为先验分布，其实就是DDPM最后的标准正态高斯噪声。
...
**“模型似乎学会了语义，但它并非必然，这只是优化动力学 + 数据结构共同导致的副产物。”** 从直觉上讲，可以把扩散模型理解成一种“空间重塑”。真实图片在像素空间里的分布非常奇怪，像是一块崎岖不平、形状离散的地形：有些区域几乎从不出现，有些区域密度极高。扩散过程的作用，就是把这块复杂的“概率地形”逐渐揉平、摊开，最后把整个空间压成一个规整、光滑、各向同性的标准高斯分布。这个高斯空间的维度与原图相同，但结构简单得多，可以随意取样，也可以保证任意一点都有合理的“意义”。反向生成就是沿着学到的轨迹，把高斯空间里的一点重新折回原始的像素分布，于是每个随机噪声都能对应一张自然图像。
这样一来，生成模型在做的事情可以概括成：把原本奇形怪状的数据分布重塑成标准高斯；再把标准高斯的点通过逆变形变回真实图像。这个标准高斯分布就是整个模型的“统一起点”，也是扩散模型能够稳定生成图像的核心原因。
