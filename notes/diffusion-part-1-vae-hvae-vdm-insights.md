...
但是我们把问题简化成如下版本： 
1. z 不被看成“抽象 latent”，而应该被看成图像的文字描述，因为文字是低维表征，是更容易理解的直觉模型。
2. $p(x \mid z)$ 给定文字之后的图像分布, 如果分布已知，可以通过文字生成图像，这里x的自由度就是图片的dimension。例如每个pixel的值满足高斯分布，那么p的极大似然就是当前输入文字生成的图片。
