> Scope: this note only addresses **Δlog p(x_t)** (the score at noise level t). No classifier guidance discussion. No Stable Diffusion specifics. Focused on the core inference geometry.
...
This is the score at time $t$—the Δlog p term—the direction pointing uphill in log-density of the noisy distribution. It tells us how to move $x_t$ locally to increase probability under $p_t$, which is exactly the vector field an inference sampler needs.
...
So Δlog p is not mysterious: it is “how far $x_t$ is from the Bayes denoiser,” expressed as a gradient.
...
## 5. Why this addresses Δlog p even though $p_t$ is unknown
...
Thus the Δlog p vector field is (up to scaling) the same as the denoising or noise prediction target used in DDPM training. Writing things in Δlog p language does not change training; it changes the interpretation: we are learning a score field, not just a noise term.
...
Δlog p is the posterior mean denoiser expressed as a gradient field. It appears as soon as we differentiate the marginal $p_t(x_t)$.

---

### Notes on terminology

- “DataLogP” transcription errors should be read as Δlog p or score.
- When papers write “score,” they mean $\nabla_x \log p(x)$, not a probability.

replace, 我都是正三角方法,改成倒三角梯度
