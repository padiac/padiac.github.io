...
**Expected KL step length.** The same scalar reappears as the step size along the geodesic: $\mathbf{E}_{X}\!\left[D_{\mathrm{KL}}\!\left(p(\theta \mid X)\,\Vert\,\pi(\theta)\right)\right]$. Maximizing this expectation makes the posterior travel as far as possible (on average) away from the prior. Geometrically, the data's "update direction" becomes as independent as possible from the prior's information direction, so your phrase "find the orthogonal part the data brings" remains spot on.
