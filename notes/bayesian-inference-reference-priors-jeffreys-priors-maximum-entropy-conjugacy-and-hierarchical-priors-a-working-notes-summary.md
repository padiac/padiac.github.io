...
**Reference prior (information gain):** maximizes $\mathbb{E}_x\bigl[D_{KL}(p(\theta \mid x) \| \pi(\theta))\bigr]$. It requires an **order**: declare a **parameter of interest** and treat the rest as **nuisance** to be marginalized. Different orders can give different priors (unlike Jeffreys).
...
In information geometry, Fisher information defines a metric. Maximizing $\mathbb{E}_x D_{KL}(p(\theta \mid x) \| \pi(\theta))$ makes the posterior move as far as possible (on average) away from the prior along geodesics induced by the data. Geometrically, the data's "update direction" is as independent as possible from the prior's information direction, so your phrase "find the orthogonal part the data brings" is on point.
