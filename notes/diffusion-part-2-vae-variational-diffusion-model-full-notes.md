...
- 最终目标：给定训练集，通过变分下界，把
  ...
  \[
  \mathbb E\bigl\|\varepsilon - \varepsilon_\theta(x_t,t)\bigr\|^2.
  \]
...
- 对 \(t = 1,2,\dots,T\)：
  \[
  q(x_t \mid x_{t-1})
  = \mathcal N\!\bigl(x_t \mid \sqrt{\alpha_t}\,x_{t-1},\; (1-\alpha_t)I\bigr),
  \]
  其中每一步的噪声都是独立的高斯：
  \[
  \varepsilon_t \sim \mathcal N(0,I),\quad
...
\[
q(x_{0:T})
= p_{\text{data}}(x_0)\,\prod_{t=1}^T q(x_t \mid x_{t-1}).
\]
...
\[
q(x_t \mid x_0)
= \mathcal N\!\bigl(x_t \mid \sqrt{\bar\alpha_t}\,x_0,\; (1-\bar\alpha_t)I\bigr),
...
- 条件期望：
  \[
  \mathbb E[x_1 \mid x_0] = \sqrt{\alpha_1}\,x_0.
  \]
- 条件方差：
  \[
  \mathrm{Var}(x_1\mid x_0)
  = (1-\alpha_1)\,\mathrm{Var}(\varepsilon_1)
  = (1-\alpha_1)I.
  \]
...
\begin{aligned}
x_2
&= \sqrt{\alpha_2}\,x_1 + \sqrt{1-\alpha_2}\,\varepsilon_2 \\
&= \sqrt{\alpha_2}\bigl(\sqrt{\alpha_1}\,x_0 + \sqrt{1-\alpha_1}\,\varepsilon_1\bigr)
  + \sqrt{1-\alpha_2}\,\varepsilon_2 \\
&= \sqrt{\alpha_1\alpha_2}\,x_0
 + \sqrt{\alpha_2(1-\alpha_1)}\,\varepsilon_1
 + \sqrt{1-\alpha_2}\,\varepsilon_2.
\end{aligned}
\]
...
\begin{aligned}
-\log p_\theta(x_0)
&\le -\mathbb E_q\bigl[\log p_\theta(x_{0:T}) - \log q(x_{1:T}\mid x_0)\bigr] \\
&= \underbrace{\mathrm{KL}\bigl(q(x_{1:T}\mid x_0)\,\Vert\, p_\theta(x_{1:T}\mid x_0)\bigr)}_{\text{“路径 KL”}}
   - \log p_\theta(x_0) + \text{常数},
\end{aligned}
\]
...
\Bigl[ \log q(x_{1:T}\mid x_0) - \log p_\theta(x_{0:T}) \Bigr].
\]
...
\[
\mathbb E_{q(x_{0:T})}\Bigl[\log \frac{q(x_t\mid x_{t-1})}{p_\theta(x_{t-1}\mid x_t)}\Bigr].
\]
...
\[
\mathbb E_{q(x_{0:T})}[f(x_{t-1},x_t)]
= \mathbb E_{q(x_{t-1:t})}[f(x_{t-1},x_t)]
= \iint f(x_{t-1},x_t)\,q(x_{t-1},x_t)\,dx_{t-1}dx_t.
\]
...
\[
\mathbb E_{q(x_{0:T})}[\cdots]
\;\longrightarrow\;
\mathbb E_{q(x_{t-1:t}\mid x_0)}[\cdots],
\]
...
\[
\mathbb E_{q(x_{t-1:t}\mid x_0)}
\Bigl[
\log q(x_{t-1}\mid x_t,x_0) - \log p_\theta(x_{t-1}\mid x_t)
\Bigr].
\]
...
&= \mathbb E_{q(x_t\mid x_0)}
   \underbrace{
     \mathbb E_{q(x_{t-1}\mid x_t,x_0)}
     \bigl[\log q(\cdot) - \log p_\theta(\cdot)\bigr]
   }_{\mathrm{KL}\bigl(q(x_{t-1}\mid x_t,x_0)\Vert p_\theta(x_{t-1}\mid x_t)\bigr)}.
\end{aligned}
\]
...
\[
\mathcal L
= \underbrace{\mathrm{KL}(q(x_T\mid x_0)\Vert p_\theta(x_T))}_{\text{prior matching term}}
+ \underbrace{\mathbb E_q[-\log p_\theta(x_0\mid x_1)]}_{\text{reconstruction term}}
+ \sum_{t=2}^T \underbrace{\mathbb E_q\mathrm{KL}\bigl(q(x_{t-1}\mid x_t,x_0)\Vert p_\theta(x_{t-1}\mid x_t)\bigr)}_{\text{transition matching}}.
\]
...
\propto q(x_t\mid x_{t-1},x_0)\,q(x_{t-1}\mid x_0).
\]
...
- 真实的反向后验：
  \[
  q(x_{t-1}\mid x_t,x_0)
  = \mathcal N\!\bigl(x_{t-1}\mid \mu_q(x_t,x_0),\; \Sigma_q\bigr),
  \]
...
- 模型的反向分布：
  ...
  = \mathcal N\!\bigl(x_{t-1}\mid \mu_\theta(x_t,t),\; \Sigma_\theta(t)\bigr).
  \]
...
\[
\mathrm{KL}(q\Vert p)
= \frac12 (\mu_p-\mu_q)^\top\Sigma_t^{-1}(\mu_p-\mu_q).
\]
...
- 于是：
  \[
  \mathrm{KL}(q\Vert p)
  = \frac{1}{2\sigma_t^2}\|\mu_p-\mu_q\|^2.
  \]
...
\mathrm{KL}\bigl(q(x_{t-1}\mid x_t,x_0)\Vert p_\theta(x_{t-1}\mid x_t)\bigr)
= \frac{1}{2\sigma_t^2}
\bigl\|\mu_q(x_t,x_0) - \mu_\theta(x_t,t)\bigr\|^2 + \text{常数}.
\]
...
\[
q(x_{t-1}\mid x_t,x_0)
= \mathcal N\!\bigl(x_{t-1}\mid \mu_q(x_t,x_0),\,\Sigma_q(t)\bigr),
\]
...
\propto \bigl\|\varepsilon - \varepsilon_\theta(x_t,t)\bigr\|^2.
\]
...
1. 用 closed-form 写
   \[
   \mu_q(x_t,x_0) = \frac{1}{\sqrt{\alpha_t}}\Bigl(
...
3. 两者相减：
   ...
   = -\frac{1-\alpha_t}{\sqrt{\alpha_t}\sqrt{1-\bar\alpha_t}}
     \bigl(\varepsilon - \varepsilon_\theta(x_t,t)\bigr).
   \]

4. L2：
   \[
   \bigl\|\mu_q - \mu_\theta\bigr\|^2
   = C_t^2\,\bigl\|\varepsilon - \varepsilon_\theta(x_t,t)\bigr\|^2,
   \]
   其中 \(C_t\) 只依赖于时间步和调度。
...
\boxed{
L(\theta)
\propto \sum_{t=1}^T w_t\,
\mathbb E_{x_0,\varepsilon}\bigl\|\varepsilon - \varepsilon_\theta(x_t,t)\bigr\|^2
}
\]
...
3. **预测噪声** \(\varepsilon_\theta(x_t,t)\)：  
   ...
   让 loss 变成 \(\|\varepsilon - \varepsilon_\theta(x_t,t)\|^2\)。
...
5. 用 loss：
   \[
   L_t(\theta)
   = \bigl\|\varepsilon - \varepsilon_\theta(x_t,t)\bigr\|^2
   \]
   或带权重的版本 \(w_t L_t\)。
...
- 就能用它来还原“干净图像” \(\mathbb E[x_0\mid x_t]\)。
...
5. 噪声 parameterization：\(\mu_q\) 写成 \(\varepsilon\) 的仿射函数，从而“把 KL 变成 \(\|\varepsilon - \varepsilon_\theta\|^2\)”
